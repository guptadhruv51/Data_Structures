...
function showMeme() {
  ...
  const div=document.querySelector(".meme-content");
  const img=document.createElement("img");
  img.setAttribute("src",randomMemeUrl);
  img.setAttribute("alt","Couldn't load the Meme");
  clearAll();
  div.appendChild(img);
...
function showJoke() {
  ...
  const div=document.querySelector(".joke-content");
  const para=document.createElement("p");
  para.textContent=randomJokeText;
  const isPara=div.querySelector("p");
  clearAll();
...
function showRiddle() {
  ...
  const div=document.querySelector(".riddle-content");
...
function revealAnswers() 
{
    ...
    else if(riddle)
    {
      ...
      alert("There is no riddle to reveal to answer to");
    }
